...
<div class="relative group" x-data="{ deliveriesOpen: {{ request()->routeIs('superadmin.deliveries.*') ? 'true' : 'false' }} }" x-init="$watch('open', val => { if (!val) deliveriesOpen = false })">
    <button @click="if(open) deliveriesOpen = !deliveriesOpen"
        ...
        <i class="fa-solid" :class="deliveriesOpen ? 'fa-chevron-up' : 'fa-chevron-down'" x-show="open"></i>
    </button>
    ...
    <!-- Child links -->
<div x-show="deliveriesOpen && open" x-collapse class="pl-10 mt-2 space-y-1 border-l border-white/20 ml-2">
    <a href="{{ route('superadmin.deliveries.index') }}"
       class="flex items-center gap-2 text-sm py-2 px-4 rounded-lg transition-all duration-200 mb-2.5
