@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex justify-center mt-6">
        <ul class="inline-flex items-center space-x-1 rounded-md bg-white shadow-md px-2 py-1 border border-gray-200">
            ...
            @else
                <li>
                    <a href="{{ $paginator->previousPageUrl() }}" class="ajax-pagination flex justify-center items-center px-3 py-2 text-gray-700 hover:bg-[#4A90E2] hover:text-white rounded-md transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                        </svg>
                    </a>
            ...
            @foreach ($elements as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <li>
                                <a href="{{ $url }}" class="ajax-pagination px-3 py-2 text-gray-700 hover:bg-[#4A90E2] hover:text-white rounded-md transition">{{ $page }}</a>
                            </li>
                        @endif
                    @endforeach
                @endif
    ...
    @if ($paginator->hasMorePages())
        <a href="{{ $paginator->nextPageUrl() }}" class="ajax-pagination flex items-center justify-center w-10 h-10 text-gray-700 hover:bg-[#4A90E2] hover:text-white rounded-md transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
